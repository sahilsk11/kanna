...
import type { AgentProvider, ChatAttachment, TranscriptEntry } from "../shared/types"
...
describe("AgentCoordinator cursor integration", () => {
  test("binds cursor provider and reuses the session token on later turns", async () => {
    const sessionCalls: Array<{ chatId: string; sessionToken: string | null }> = []
    const turnCalls: Array<{ content: string; model?: string; planMode?: boolean }> = []

    const fakeCursorManager = {
      async startSession(args: { chatId: string; sessionToken: string | null }) {
        sessionCalls.push({ chatId: args.chatId, sessionToken: args.sessionToken })
        return args.sessionToken ?? "cursor-session-1"
      },
      async startTurn(args: { content: string; model?: string; planMode?: boolean }): Promise<HarnessTurn> {
        turnCalls.push({ content: args.content, model: args.model, planMode: args.planMode })
        ...
        async function* stream() {
          ...
          yield {
            ...
            entry: timestamped({
              kind: "system_init",
              provider: "cursor",
              model: args.model ?? "cursor-configured-default",
              tools: [],
              agents: [],
              slashCommands: [],
              mcpServers: [],
    ...
    expect(sessionCalls).toEqual([
      { chatId: "chat-1", sessionToken: null },
      { chatId: "chat-1", sessionToken: "cursor-session-1" },
    ])
    expect(turnCalls.map((call) => call.content)).toEqual(["first", "second"])
  })

  test("maps cursor plan mode into Cursor turn settings", async () => {
    const turnCalls: Array<{ planMode?: boolean }> = []
    const fakeCursorManager = {
      async startSession() {
        return "cursor-session-1"
      },
      async startTurn(args: { planMode?: boolean }): Promise<HarnessTurn> {
        turnCalls.push({ planMode: args.planMode })
    ...
    await waitFor(() => store.turnFinishedCount === 1)
    expect(turnCalls).toEqual([{ planMode: true }])
    expect(store.chat.planMode).toBe(true)
  })

  test("cancels cursor active turn through CursorManager interrupt", async () => {
...
function createFakeStore() {
  ...
  return {
    ...
    async setChatProvider(_chatId: string, provider: AgentProvider) {
